/**
 * MboardPage — M-Board (interactive whiteboard)
 * ─────────────────────────────────────────────────────────────────────────────
 * Navigation: Dashboard → TO-DO → Start Class → (M-Board loads)
 * URL:        /todoin/<classId>
 *
 * The M-Board is the live classroom canvas.
 * Sidebar tabs: Recap | e-Content | e-Resource | Quiz | FAQ
 * e-Content popover contains YouTube, AR, and other media icons.
 */
class MboardPage {
  /**
  ...
  /** Wait for the M-Board canvas to be visible (e-Content tab is reliable indicator) */
  async waitForLoad(timeout = 20000) {
    await this.eContentTab.waitFor({ state: 'visible', timeout });
  }

  /** Click the e-Content sidebar tab to open the media popover */
  async openEContent() {
  ...
  /** Wait until the YouTube embed iframe is visible (video is playing) */
